...
class Database{
    ...
    findOne = (param, value, collectionName, projection = null) =>{
        if(!projection){
            return this.getDatabase().collection(collectionName).findOne({[param]: value})
        }
        else{
            return this.getDatabase().collection(collectionName).findOne({[param]: value}, projection)
        }
    }
    deleteOne = (param, value, collectionName)=>{
        return this.getDatabase().collection(collectionName).deleteOne({[param]: value})
    }
