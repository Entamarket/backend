...
middleware.updateUploads = router.put ('/product/update-product', async(req, res, next)=>{
    ...
    try{
        ...
        const shopObj = await database.findOne({_id: ObjectId(req.query.shopID)}, database.collection.shops, ['owner'], 1)
        ...
        if(decodedToken.userID === shopObj?.owner.toString() && decodedToken.userID === productObj?.owner.toString() && shopObj?._id.toString() === productObj?.shopID.toString()){
