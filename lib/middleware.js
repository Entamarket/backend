...
///////////////////////////////////////////////////////////////////////////////
//const traderVerificationDocs = {
    const traderVerificationStorage = multer.diskStorage({
        destination: async(req, file, cb)=>{
            const decodedToken = req.decodedToken
            ...
            try{
                ...
                const userID = req.decodedToken.userID
                //check if idDocs exists
                if (!fs.existsSync(path.join('multimedia', 'traders', userID, 'idDocs'))) {
                    //create folder for documents   
                    fs.mkdirSync(path.join('multimedia', 'traders', userID, 'idDocs'))
                    cb(null, path.join('multimedia', 'traders', userID, 'idDocs'))
                }
                else{
                    if(fs.readdirSync(path.join('multimedia', 'traders', userID, 'idDocs')).length <= 1){
                        cb(null, path.join('multimedia', 'traders', userID, 'idDocs'))
                    }
                    else{
                        ...
                        if(verificationDocs && verificationDocs.verified){
                            cb({message:"your verification documents are approved"})
                        }
                        else{
                            cb({message:"your verification documents are being reviewed, please be patient"})
                        }  
                    }
                }
    ...
    const traderVerificationUpload = multer({
        storage: traderVerificationStorage,
        limits: {fileSize: Math.pow(2, 20) * 6},
        fileFilter: fileFilter
    }).fields([{name: "idCard"}, {name: "utilityBill"}])
...
middleware.uploadTraderVerificationDocs = router.post('/trader/upload-verification-docs', async (req, res, next)=>{
    try{
    
        
        if(true){
            
            traderVerificationUpload(req, res, (err)=>{
    
                if(err){
                
                    utilities.setResponseData(res, 201, {'content-type': 'application/json'}, {statusCode: 201, msg: err.message}, true )
                }
                else{
                    
                    if(req.files.idCard && req.files.utilityBill){
                        next()
                    }
                    else{
                        
                        utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: 'no pictures added, make sure you add pictures of the necessary documents'}, true )
                    }
